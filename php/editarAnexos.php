...
$consulta = "SELECT anexouno, anexodos, anexotres, anexocuatro FROM hclinica WHERE idhc = '" . $idhc . "' ";
$resultado = mysqli_query($conexion, $consulta);
if ($resultado) {
    while ($row = $resultado->fetch_array()) {
        $anexouno = $row["anexouno"];
        $anexodos = $row["anexodos"];
        $anexotres = $row["anexotres"];
        $anexocuatro = $row["anexocuatro"];
    }
}
