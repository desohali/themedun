...
if (explode("/", $_GET['view'])[1]==''){
    echo "<script>window.location.href='" . $_ENV['APP_URL'] . "administradores'</script>";
}
...
if (isset($_GET['id']) && @$_GET['id']==@$administradoresRegistrados){
    ...
    //header("Location: ".$_ENV['APP_URL']."administradores");
    echo "<script>window.location.href='" . $_ENV['APP_URL'] . "administradores'</script>";
}
        ?>
<!DOCTYPE html>
...
<a href="https://api.whatsapp.com/send?phone=51986206045&text=Hola,%20tengo%20una%20consulta%20%C2%BFpueden%20ayudarme?%20%F0%9F%A4%94" target="_blank" class="btn-wsp"><i class="fa-brands fa-whatsapp"></i></a>
    ...
    <script>
        window.addEventListener("DOMContentLoaded", function(){
            ...
            formDesbloquear.addEventListener("submit", async function(e){
                ...
                const {isConfirmed} = await Swal.fire({
                    ...
                    text: "¿Está seguro de desbloquear esta cuenta de administrador?",
                    icon: 'question',
                ...
                if (isConfirmed) {
                    ...
                    const response = await fetch("<?php echo $_ENV['APP_URL'];?>php/desbloquearCuentaAdmin.php", {
                        method: "post",
                        body: formData
                    });
                        const text = await Swal.fire({
                            title: 'Cuenta de administrador desbloqueada',
                            text: '',
                            icon: 'success',
        ...
        window.addEventListener("DOMContentLoaded", function(){
            ...
            formBloquear.addEventListener("submit", async function(e){
                ...
                const {isConfirmed} = await Swal.fire({
                    title: 'Cuenta de administrador',
                    text: "¿Está seguro de bloquear esta cuenta de administrador?",
                    icon: 'question',
                    showCancelButton: true,
                ...
                if (isConfirmed) {
                    ...
                    formData.append("id", <?php echo $id;?>);
                    formData.append("idadmin", <?php echo $_SESSION['idAdmin'];?>);
                    const response = await fetch("<?php echo $_ENV['APP_URL'];?>php/bloquearCuentaAdmin.php", {
                        method: "post",
                        body: formData
                    });
                        const text = await Swal.fire({
                            title: 'Cuenta de administrador bloqueada',
                            text: '',
                            icon: 'success',
                            confirmButtonColor: '#0052d4',
                            confirmButtonText: 'Ok',
    ...
    <div class="filauno" id="filapaciente">
        ...
        <?php
                ...
                <form id="formBloquear" method="post">
                    ...
                    <hr id="edit-infb">
                    <textarea style="margin-bottom:0px" name="observaciones" placeholder='Observaciones a corregir por el administrador.' rows="5" maxlength="500" required></textarea>
                </form>
                </div>
                <?php
                }
    ...
    if($estado == '2'){
    ?>
    <div class="revision" id="revisionpac">
        <h2>CUENTA BLOQUEADA...</h2>
        <p>The Med Universe se reserva la posibilidad de bloquear al usuario administrador, por cuestiones de seguridad y/o alguna infracción de los Términos y Condiciones de uso de la plataforma y/o acuerdos consignados en su contratación.</p>
    </div>
    <div class="filatres">
        ...
        if(isset($_POST['editip3'])){
            ...
            $sql9 = mysqli_query($conexion, "UPDATE administradores SET indicacionesAdmin = '$observaciones3' WHERE idAdmin = '".$id."' ");
            if ($sql9) {
                $url = "<script>window.location.href='" . $_ENV['APP_URL'];
                echo $url . "perfiladministrador/" . $id . "'</script>";
            }
        }
        ?>
    </div>
