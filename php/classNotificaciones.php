...
class Notificaciones
{
  ...
  /* public function listarNotificacionesRespuestaComentario()
  {
    list(
      'id' => $id,
    ) = @$_POST;

    $query = "SELECT * FROM valoraciones WHERE idu=" . $id . " AND valoracion=0 AND comentarios REGEXP '[(visto)\\\S{1,}:\\\S{1,}NO]'";
  ...
  public function listarNotificacionesComentariosPaciente()
  {
    $query = "SELECT *, (select estado from usuarios where id=" . @$_POST['id'] . ") as estado FROM valoraciones ";
    $query .= "WHERE idu=" . @$_POST['id'] . " and valoracion=0"; //  AND comentarios REGEXP '(leido)\\\S{1,}(NO)'
  ...
  public function actualizarNotificacionesLeidasNoLeidas()
  {
    ...
    while ($obj = $result->fetch_object()) {
      ...
      for ($i = 0; $i < count($obj->comentarios); $i++) {
        ...
        $comentario = json_encode(preg_replace('/(visto)\S{1,}(NO)/', "visto\":\"SI", $obj->comentarios[$i]), JSON_UNESCAPED_UNICODE);
        if ($i == 0) {
